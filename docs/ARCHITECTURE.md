...
- **Responsibility**: Assemble classified bundle content into an LLM prompt. Truncate to fit within the 100K token budget using priority-based strategy.
...
- **Responsibility**: Manage chat conversation flow. Provide report + manifest as context. Handle `get_file_contents` tool calls by retrieving file content from session store.
- **Interfaces**: `handle_chat(session: Session, message: str) -> AsyncIterator[str]`
- **Dependencies**: LLM provider, session store.
...
- **Session** — session_id, created_at, bundle_manifest, extracted_files, classified_signals, report, chat_history
